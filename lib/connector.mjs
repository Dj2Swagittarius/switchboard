...
// ---- who is connected ---------------------------------------------------------------
// For Settings → Check connection: the last Claude app that reached us with
// the right token, and the last attempt with a wrong one. Kept on disk so it
// survives a restart of Switchboard.
const SEEN = dataPath('connector-seen.json');
const ACTIONS = { hello: 'connected', instructions: 'connected', pending: 'looked for waiting texts',
  triage: 'saved triage', thread: 'read a conversation', media: 'viewed a photo', day: 'read a day for the recap',
  recap: 'saved a recap' };
function readSeen() { try { return JSON.parse(readFileSync(SEEN, 'utf8')); } catch { return {}; } }
function writeSeen(patch) { try { writeFileSync(SEEN, JSON.stringify({ ...readSeen(), ...patch })); } catch {} }
const clientOf = (req) => String(req.headers['x-switchboard-client'] ?? '').replace(/[^\w .@/+-]/g, '').slice(0, 80) || 'unknown';

export function noteContact(req, route) {
  writeSeen({ ok: { at: new Date().toISOString(), client: clientOf(req), did: ACTIONS[route] ?? route } });
}

// Claude Desktop says "claude-ai", Claude Code "claude-code".
function appName(client) {
  if (/^claude-code/i.test(client)) return 'Claude Code';
  if (/^claude-ai|^claude desktop/i.test(client)) return 'Claude Desktop';
  return client === 'unknown' ? 'A Claude app' : client;
}

export function status() {
  let ready = true;
  try { token(); } catch { ready = false; }
  const s = readSeen();
  return {
    ready,
    last: s.ok ? { ...s.ok, app: appName(s.ok.client) } : null,
    rejected: s.bad && (!s.ok || s.bad.at > s.ok.at) ? { ...s.bad, app: appName(s.bad.client) } : null,
  };
}
...
export function authorized(req) {
  ...
  if (!ok) writeSeen({ bad: { at: new Date().toISOString(), client: clientOf(req) } });
  return ok;
}
