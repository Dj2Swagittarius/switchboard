...
// Adds (or replaces) an MCP server in Claude Code's user config, as
// `claude mcp add` would. The earlier entry is removed first: `add` refuses a
// name that exists, and the old one may hold a token that no longer works.
function cli(args) {
  const exe = find();
  if (!exe) return Promise.reject(new Error(NOT_FOUND));
  const [cmd, argv] = command(exe, args);
  return new Promise((resolve, reject) => {
    const child = execFile(cmd, argv, { timeout: 30e3, windowsHide: true }, (e, stdout, stderr) => {
      if (e) return reject(new Error(String(stderr || stdout || e.message).trim().slice(0, 240)));
      resolve(String(stdout));
    });
    child.stdin?.end();
  });
}
export async function addMcp({ name, env, command: exe, args }) {
  await cli(['mcp', 'remove', name, '-s', 'user']).catch(() => {});
  const flags = Object.entries(env).flatMap(([k, v]) => ['-e', `${k}=${v}`]);
  await cli(['mcp', 'add', name, '--scope', 'user', ...flags, '--', exe, ...args]);
}
...
export function status() {
  ...
  return new Promise((resolve) => {
    const child = execFile(cmd, argv, { timeout: 5000, windowsHide: true }, (e, stdout) => {
