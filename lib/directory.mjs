...
async function buildTraffic(session, days) {
  ...
  for (const c of cdrs) {
    ...
    const other = inbound ? c.caller_id_number : c.callee_id_number;
    const r = bump(other);                         // internal extensions fall out here
    if (!r) continue;
    r.calls++;
    r.lastSeen = Math.max(r.lastSeen, at);
    // The other party's own name. On a leg where they're the one called, the
    // caller is one of this account's phones, often named after its device
    // ("desktop_…"), which must never become their name.
    suggest(r, inbound ? c.caller_id_name : c.callee_id_name);
  }
