...
//   a conversation:  DELETE {msg}/v2/messaging  {data:{remoteNumber:[thread id], localNumber, updatedBy}}
//                    (thread id: the other numbers, E.164, sorted and comma-joined)
...
export async function deleteThread(session, { remotes, localNumber }) {
  if (!Array.isArray(remotes) || !remotes.length || !remotes.every(n => /^\+\d{8,15}$/.test(n))) throw new Error('bad number');
  if (!/^\+\d{8,15}$/.test(localNumber ?? '')) throw new Error('bad localNumber');
  const base = derived().messagingServer;
  if (!base) throw new Error('No messaging server is set.');
  const updatedBy = await resolvePostedBy(session);
  return api(session, `${base}/v2/messaging`,
    { method: 'DELETE', body: JSON.stringify({ data: { remoteNumber: [[...remotes].sort().join(',')], localNumber, updatedBy } }) });
}
