...
// ---- storage -------------------------------------------------------------------
const file = (day) => dataPath('reviews', day + '.json');
...
- watch: conversations with frustration, complaints or conflict. Empty array
  if none.
No emoji. No filler.`;
export const RECAP_INSTRUCTIONS = SYSTEM;
...
// Everything the recap is written from: stats, and the material as text at a
// level of detail (see below). `step` reports progress; problems with single
// items go into `errors` and don't stop the rest.
async function collect(session, day, { transcribe = settings.get('review.transcribe'), step = () => {}, errors = [] } = {}) {
  step('Collecting texts…');
  const [from, to] = bounds(day);
  const inDay = (t) => t >= from && t < to;
  const perItem = ai.bigContext(ai.provider()) ? 12000 : 2500;
  ...
  for (const [i, v] of pendingVm.entries()) {
    if (!transcribe || budget <= 0) break;
    step(`Transcribing voicemail ${i + 1} of ${pendingVm.length}…`);
  ...
  if (!ai.bigContext(ai.provider())) {
    ...
    for (const [n, c] of busy.entries()) {
      ...
      for (let i = 0; i < c.today.length; i += CHUNK) {
        step(`Reading texts with ${c.name || c.remote} (${n + 1} of ${busy.length}, part ${i / CHUNK + 1} of ${parts})…`);
  ...
  const empty = !msgCount && !calls.length && !vms.length;
  ...
  return { base, empty, material };
}

export async function run(session, day = yesterday(), { onDone } = {}) {
  if (state.running) throw new Error('A recap is already being written.');
  Object.assign(state, { running: true, day, step: '', error: null });
  const errors = [];
  try {
    const { base, empty, material } = await collect(session, day, { errors, step: (s) => { state.step = s; } });
    if (empty) {
      const r = { ...base, empty: true, recap: null };
      save(r); onDone?.(r); return r;
    }
    const local = !ai.bigContext(ai.provider());
    let level = 0;
    if (local) {
    ...
    for (;;) {
      ...
      try {
        recap = await ai.json({
          name: 'recap', schema: RECAP_SCHEMA, system: SYSTEM, user: material(level),
...
export function due() {
  // Connector mode: the Claude app writes the recap when asked (daily-recap).
  if (!settings.get('review.enabled') || state.running || ai.provider() === 'connector') return false;
...
// ---- connector mode --------------------------------------------------------------
// The Claude app writes the recap: it asks for the material (get_day) and
// sends back the answer (save_recap). Nothing is transcribed here, so the call
// returns quickly; calls and voicemail already transcribed are included.
const collected = new Map();   // day -> base, so the saved recap keeps the stats

export async function connectorDay(session, day) {
  const errors = [];
  const { base, empty, material } = await collect(session, day, { transcribe: false, errors });
  collected.set(day, base);
  return { day, empty, stats: base.stats, errors, instructions: SYSTEM, schema: RECAP_SCHEMA,
           material: empty ? '' : material(0) };
}

export async function saveConnectorRecap(session, day, recap) {
  const errors = validate(RECAP_SCHEMA, recap);
  if (errors.length) return { ok: false, errors };
  let base = collected.get(day);
  if (!base) base = (await collect(session, day, { transcribe: false })).base;
  const r = { ...base, generated_at: new Date().toISOString(), provider: 'connector', model: 'Claude app',
              empty: false, recap, detail: 0 };
  save(r);
