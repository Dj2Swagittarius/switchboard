...
import { sendMessage, uploadMedia, MMS_TYPES, MMS_MAX_BYTES, MMS_MAX_FILES } from './send.mjs';
...
export async function sendText(session, { line, to, text, media = [] }) {
  const L = myLine(line);
  const T = e164(to);
  ...
  const r = await sendMessage(session, { to: T, text: body, localNumber: L, media, dryRun: false });
  ...
  const cid = r.body.data.clientMsgId;
  const sent = (Array.isArray(r.response?.data) ? r.response.data : [r.response?.data]).find(m => m?.clientMsgId === cid);
