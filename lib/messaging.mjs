...
// Read marks. The platform's unread count only drops when its own app opens a
// thread, so opening one here is remembered locally: the newest message then
// shown, and the platform's count at that time. A thread shows as unread again
// only when a newer message comes in, and then only for the new ones.
// rawUnread: the platform's count; unread: what is shown.
function applyRead(session, c) {
  const r = cache.getRead(session, c.line, c.remote);
  if (!r) return { ...c, unread: c.rawUnread };
  if (c.lastAt <= r.at || !c.last.inbound) return { ...c, unread: 0 };
  const raw = c.rawUnread ?? 0;
  return { ...c, unread: Math.max(1, raw >= r.raw ? raw - r.raw : raw) };
}

// Called when Messages shows a thread: everything in it counts as read.
export function markRead(session, { line, remote, messages }) {
  const L = e164(line), R = e164(remote);
  if (!L || !R || !messages?.length) return;
  const at = messages.reduce((n, m) => Math.max(n, m.at || 0), 0);
  let raw = 0;
  for (const k of ['', L]) {
    const c = cache.getList(session, k)?.conversations?.find(c => c.line === L && c.remote === R);
    if (c) { raw = c.rawUnread ?? c.unread ?? 0; break; }
  }
  const was = cache.getRead(session, L, R);
  if (was && was.at >= at && was.raw === raw) return;
  cache.putRead(session, L, R, { at: Math.max(at, was?.at ?? 0), raw });
}
...
export async function conversations(session, { line = null, cached = false } = {}) {
  ...
  if (cached) return cache.getList(session, lineKey)?.conversations?.filter(c => which.includes(c.line))
    .map(c => applyRead(session, { ...c, rawUnread: c.rawUnread ?? c.unread })) ?? null;
  ...
  // Nothing new since a thread was read: take the platform's current count as
  // the new baseline, so the next message counts as one, not as all of these.
  for (const c of list) {
    const r = cache.getRead(session, c.line, c.remote);
    if (r && c.lastAt <= r.at && r.raw !== c.rawUnread) cache.putRead(session, c.line, c.remote, { ...r, raw: c.rawUnread });
  }
  return list.map(c => applyRead(session, c));
}
