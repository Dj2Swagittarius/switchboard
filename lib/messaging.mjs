...
import { sendMessage, uploadMedia, deleteAttachment, deleteMessages, MMS_TYPES, MMS_MAX_BYTES, MMS_MAX_FILES } from './send.mjs';
...
// Deletes one photo (or other attachment) from a conversation, the way the
// platform's own app decides it: if the message also has text or other
// attachments, just this one goes; if it's all the message is, the message
// goes. The message must be in the saved copy of this conversation, which is
// then updated to match.
export async function deleteMedia(session, { line, remote, messageId, mediaId }) {
  const L = myLine(line);
  const R = e164(remote);
  if (!R) throw new Error('bad number');
  if (!safeMediaId(mediaId) || !safeMediaId(messageId)) throw new Error('bad id');
  const have = cache.getThread(session, L, R);
  const m = have?.messages?.find(x => x.id === messageId);
  if (!m || !m.media.some(x => x.id === mediaId)) throw new Error('that photo isn’t in this conversation any more (refresh and try again)');
  const whole = m.media.length === 1 && !m.text;
  ...
  return { ok: true, removed: whole ? 'message' : 'attachment' };
}
