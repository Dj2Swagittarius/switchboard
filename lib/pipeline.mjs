...
import * as connector from './connector.mjs';
...
export async function runOnce(session, { backlog = null, skip = new Set(), onItem = null,
  ...
  for (const m of fresh) {
    ...
    // Connector mode: the Claude app triages it later (it sees the photos
    // itself); meanwhile it waits in the queue and is announced as a plain
    // new text.
    if (viaConnector) {
      const entry = connector.entryFor(m, { history, contact, rawMedia: rawById.get(m.id)?.media });
      entry.announced = isNews(m);
      if (entry.announced) announce(m);
      store.add(entry);
      done.push(entry);
      continue;
    }
